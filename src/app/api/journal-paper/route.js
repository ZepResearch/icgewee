...
export async function POST(request) {
  try {
    ...
    const record = await pb.collection("ICGEWEE_journal_form_submission").create(pbFormData)
    ...
    await resend.emails.send({
      from: "ICGEWEE |Journal-Submission <info@icgewee.com>",
      to: data.email,
      subject:
        "Journal Submission Confirmation - ICGEWEE  ",
      html: getUserEmailTemplate(data),
    })

    // Send notification email to admin
    await resend.emails.send({
      from: "ICGEWEE  | Submission <info@icgewee.com>",
      to: "info@icgewee.com", // Replace with actual admin email
      subject: "New Journal Submission - from ICGEWEE ",
