...
const SpeakerCard = ({ name, role, image, bio,collectionId, id, country, college, onMoreInfo }) => (
  ...
  >
    ...
    />
    <div className="p-4">
      <h3 className="text-xl font-semibold text-lime-600">{name}</h3>
      <p className="text-lime-900">{role}</p>
      <p className="text-sm">{college}</p>
      <p className="text-sm">{country}</p>
      <button
        onClick={() => onMoreInfo({ name, role, image, bio ,collectionId, id ,country,college}) }
        className="mt-2 px-4 py-2 bg-gradient-to-br from-yellow-400 to-lime-500 text-white rounded  transition-colors duration-300 hover:drop-shadow-lg"
      >
        More info
      </button>
    </div>
      ...
      <>
        ...
        >
          <div className="p-6 flex flex-col h-full">
            ...
            <h3 className="text-xl text-lime-900 ">{speaker.role}</h3>
            ...
            <button
              onClick={onClose}
              className="mt-6 w-full px-4 py-2 bg-gradient-to-bl from-yellow-400 to-lime-500 text-white rounded hover:bg-orange-700 transition-colors duration-300"
            >
              Close
            </button>
          </div>
  ...
  if (loading) {
    return (
      <div className=" min-h-full flex items-center justify-center">
        <div className="text-[#c5ff00] text-xl">Loading speakers...</div>
      </div>
    );
  }
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                activeCategory === category.id
                  ? "bg-[#c5ff00] text-black shadow-md"
                  : "bg-white text-[#4a5e0b] hover:bg-emerald-50"
              }`}
            >
              {category.title}
            </button>
