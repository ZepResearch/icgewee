...
export default function Buttons() {
  ...
  useEffect(() => {
    const fetchMaterials = async () => {
      try {
        const records = await pb.collection('ICGEWEE_download_material').getFullList({
          sort: 'created',
          requestKey: null
        });
  ...
  return (
    <section className="pb-14 max-w-7xl mx-auto">
      <div className="container max-w-7xl mx-auto px-4">
        <div className="flex justify-center items-center mx-auto">
          <h1 className="text-center text-3xl pb-6 inline-flex justify-center items-center gap-3 font-medium ">
        ...
        >
          {materials.map((material) => (
            <motion.a
              ...
              className="bg-neutral-950 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-colors duration-300 text-sm sm:text-sm text-center drop-shadow-sm"
